...
const weappConfig = require('../config/weapp')
...
module.exports = {
  ...
  createPassword: function (string) {
    return md5(string + weappConfig.appSecret)
  },
  ...
  mvFile: function (file, noTimesavePath) {
    const savePath    = path.join(noTimesavePath, moment(Date.now()).format('YYYY-MM-DD'))
